defmodule ExPgQuery.NormalizeTest do
  ...
  describe "normalize" do
    test "normalizes a simple query" do
      {:ok, result} = ExPgQuery.Normalize.normalize("SELECT 1")
      assert result == "SELECT $1"
    end

    test "normalizes IN(...)" do
      {:ok, result} =
        ExPgQuery.Normalize.normalize(
    ...
    test "normalizes ANY(array[...])" do
      {:ok, result} = ExPgQuery.Normalize.normalize("SELECT * FROM x WHERE y = ANY(array[1, 2])")
      assert result == "SELECT * FROM x WHERE y = ANY(array[$1, $2])"
    end

    test "normalizes ANY(query)" do
      {:ok, result} = ExPgQuery.Normalize.normalize("SELECT * FROM x WHERE y = ANY(SELECT 1)")
      assert result == "SELECT * FROM x WHERE y = ANY(SELECT $1)"
    end

    test "works with complicated strings" do
      {:ok, result} = ExPgQuery.Normalize.normalize("SELECT U&'d\\0061t\\+000061' FROM x")
      ...
      {:ok, result} =
        ExPgQuery.Normalize.normalize("SELECT * FROM x WHERE z NOT LIKE E'abc'AND TRUE")

      assert result == "SELECT * FROM x WHERE z NOT LIKE $1AND $2"

      {:ok, result} =
        ExPgQuery.Normalize.normalize("SELECT U&'d\\0061t\\+000061'-- comment\nFROM x")
    ...
    test "normalizes COPY" do
      {:ok, result} =
        ExPgQuery.Normalize.normalize("COPY (SELECT * FROM t WHERE id IN ('1', '2')) TO STDOUT")

      assert result == "COPY (SELECT * FROM t WHERE id IN ($1, $2)) TO STDOUT"
    end

    test "normalizes SETs" do
      {:ok, result} = ExPgQuery.Normalize.normalize("SET test=123")
      assert result == "SET test=$1"
    end

    test "normalizes weird SETs" do
      {:ok, result} = ExPgQuery.Normalize.normalize("SET CLIENT_ENCODING = UTF8")
      assert result == "SET CLIENT_ENCODING = $1"
    end

    test "does not fail if it does not understand parts of the statement" do
      {:ok, result} = ExPgQuery.Normalize.normalize("DEALLOCATE bla; SELECT 1")
      assert result == "DEALLOCATE bla; SELECT $1"
    end

    test "normalizes EPXLAIN" do
      {:ok, result} = ExPgQuery.Normalize.normalize("EXPLAIN SELECT x FROM y WHERE z = 1")
